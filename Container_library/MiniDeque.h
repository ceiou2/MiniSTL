...
private:
    ...
    typedef T& reference;
    typedef const T& const_reference; //为什么typedef const reference
                                      // const_reference不行？
...
private:
    ...
    { //头部添加一个buffer,start.cur指向新buffer末尾，空但将分配value，cur不移(指向第一个元素)
        ...
        // init finish :debugged
        // 这里可能会导致map重新分配空间，start和finish的地址都要更新
...
public:
    ...
    void pop_front()
    {
        if (start.cur ==
            ...
            del_p = nullptr;
            map.erase(
                    map.begin()); // erase改变map与buffer的映射关系了，要重新init
                                  // start和finish的node成员
