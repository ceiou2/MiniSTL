...
public:
    ...
    iterator insert(const_iterator pos, T&& value)
    {
        ...
        pointer tmp = new node(
                std::move(value), cur->prev,
                cur); //显示表达move中value是右值，可以不move因为已经是右值，不影响
        if (!cur->prev) {
            cur->prev->next = tmp;
        }
        cur->prev = tmp;
    ...
    iterator erase(iterator pos)
    {
        pointer cur = pos.current;
        if (!cur)
            return iterator(nullptr); //防止传入end()
    ...
    void reverse()
    {
        for (auto it = begin(); it != end();
             --it) { //这里是--因为next和prev交换后--it才是指向下一个节点
